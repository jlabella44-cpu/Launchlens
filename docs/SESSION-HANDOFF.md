...
C:\Users\Jeff\launchlens\
  src/launchlens/
    ...
    agents/
      ...
      video.py                 — VideoAgent: Kling AI image-to-video pipeline (select photos → generate clips → stitch)
      video_prompts.py         — Room prompts, camera controls, transition sequence (ported from Juke)
      chapter.py               — ChapterAgent: GPT-4V keyframe analysis → chapter markers
      social_cuts.py           — SocialCutAgent: platform-specific clips (IG, TikTok, FB, YT Shorts)
...
- **Juke Marketing Engine:** `C:\Users\Jeff\Projects\juke-marketing-engine\` — video_generator.py (already ported to LaunchLens in v0.9.2)
...
**Frontend pages completed (v1.0.1, PR open on `feat/frontend-remaining`):**
- ~~Demo dropzone landing page~~ — DONE (`/demo` + `/demo/[id]`)
- ~~Export download page with MLS vs Marketing toggle~~ — DONE (`/listings/[id]/export`)
- ~~Social media preview cards~~ — DONE (SocialPreview component on listing detail)
- ~~Pricing page with updated "Media OS" copy~~ — DONE (`/pricing`)
- VideoPlayer with chapter timeline — DONE (on listing detail)
- VideoUpload form — DONE (on listing detail)

**Frontend work remaining:**
- Fix pre-existing Three.js/R3F type error (build fails on 3D components)
- Visual QA of all pages in browser
...
### What's next:
- **Frontend QA** — Fix Three.js build error, visual QA all pages
- **Demo cleanup** — Temporal cron workflow to delete expired demo listings
- **Production deployment** — Not yet planned
- **Monitoring** — Not yet planned
...
3. ~~Video pipeline~~ — **DONE** (v0.9.2, VideoAgent, ChapterAgent, SocialCutAgent)
