...
def test_record_token_usage_without_agent():
    with patch("listingjet.services.metrics.emit_metric") as emit:
        record_token_usage("openai_gpt4v", 1000, 2000)
    assert emit.call_count == 3
