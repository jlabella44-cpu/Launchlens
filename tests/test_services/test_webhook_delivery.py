...
async def test_signature_header_present():
    ...
    assert "X-ListingJet-Signature" in headers
    assert headers["X-ListingJet-Signature"].startswith("sha256=")
    assert headers["X-ListingJet-Event"] == "test"
...
async def test_ssrf_blocked_url_returns_false():
    ...
    with patch("listingjet.services.webhook_delivery._is_url_safe", return_value=False):
        result = await deliver_webhook(
